...
#'
#' # With rate parameter
#' Etrunc_burr(d = 2, rate = 2, shape1 = 2, shape2 = 5)
#'
#' # With scale parameter
#' Etrunc_burr(d = 2, scale = 0.5, shape1 = 2, shape2 = 5)
#'
#' # Values greater than d
#' Etrunc_burr(d = 2, scale = 0.5, shape1 = 2, shape2 = 5, less.than.d = FALSE)
#'
Etrunc_burr <- function(d, shape1, shape2, rate = 1 / scale, scale = 1 / rate, less.than.d = TRUE) {
    stopifnot(shape1 > 0, shape2 > 0, rate > 0, d >= 0, shape1 * shape2 > 1)

    if (less.than.d) {
        Etrunc.burr <- E_burr(shape1, shape2, rate) *
