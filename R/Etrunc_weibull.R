...
#' Etrunc_weibull(d = 2, shape = 2, scale = 5)
#'
#' # With rate parameter
#' Etrunc_weibull(d = 2, shape = 2, rate = 0.2)
#'
#' # Mean of values greater than d
#' Etrunc_weibull(d = 2, shape = 2, rate = 0.2, less.than.d = FALSE)
#'
Etrunc_weibull <- function(d, shape, rate = 1 / scale, scale = 1 / rate, less.than.d = TRUE) {
    stopifnot(shape > 0, rate > 0, d >= 0)

    if (less.than.d) {
        Etrunc.weibull <- E_weibull(shape, rate) *
