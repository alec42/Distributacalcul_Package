...
#' @export
TVaR_BNCOMP <- function(kappa, r, q, shape, rate, vark, ko, distr_severity = "Gamma"){

    require(stats)
    if (vark == 0)
    {
        E_BNComp(r = r, q = q, shape = shape, rate = rate, distr_severity = distr_severity) / (1 - kappa)
    }
    else if (distr_severity == "Gamma")
    {
        (sum(sapply(1:ko, function(i) dnbinom(x = i, size = r, prob = q) * (shape / rate) * pgamma(q = vark, shape = shape * i + 1, rate = rate, lower.tail = F))) / (1 - kappa))
    }
    else if (distr_severity == "Lognormale")
    {
        (sum(sapply(1:ko, function(i) dnbinom(x = i, size = r, prob = q) * (shape / rate) * plnorm(q = vark, meanlog = shape * i + 1, sdlog = sqrt(rate), lower.tail = F))) / (1 - kappa))
    }
}
